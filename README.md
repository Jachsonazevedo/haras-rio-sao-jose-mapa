...
Ao tocar em "Iniciar a experiência" roda um filme de 44,5 s (`tour/img/filme.mp4`, 12,8 MB) com trechos reais do voo (portaria → avenida → rede elétrica → rede de água → glebas → chácaras habitadas → portaria) e **pagode de viola instrumental** no estilo modão (linha Tião Carreiro: ponteado em semicolcheias, dupla em terças, batida sincopada do violão; 118 bpm, 2/4, Mi maior), **composto e sintetizado** por `scripts/gerar_musica_pagode.py` (a toada calma anterior segue em `gerar_musica_caipira.py`) (sem direito autoral). O tour carrega por trás; botão "Pular"; no fim, o som ambiente do tour entra.
...
- Regerar: `python scripts/gerar_musica_pagode.py && python scripts/gerar_filme.py` → subir `filme.mp4?v=` no `tour/index.html`. Mudar tomadas = ajustar `TOMADAS` e os tempos de `FRASES` juntos.
